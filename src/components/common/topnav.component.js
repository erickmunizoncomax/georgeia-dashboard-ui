      ...
      <div x-data="{ open: false }" className="flex flex-col mx-auto md:items-center md:justify-between md:flex-row">
        ...
        <nav className="flex-col flex-grow hidden h-16 pb-4 md:flex md:items-center md:pb-0 md:justify-end md:flex-row">
          ...
          <div className="relative flex items-center gap-3 ml-5 profile-block" x-data="{ open: false }">
            <div className="px-2 baricon">
              <button onClick={() => setVisibleRight(true)}><i className="pi pi-bars"></i></button>
            </div>
            <HRProfile/>
          </div>
        </nav>
        ...
        <Sidebar visible={visibleRight} position="right" onHide={() => setVisibleRight(false)}>
          <TabView activeIndex={activeIndex} onTabChange={(e) => setActiveIndex(e.index)}>
            <TabPanel header="  " headerTemplate={BarHeaderTemplate} headerClassName="flex align-items-center">
              ...
              <div className="px-3 py-3 menu">
                <ul onClick={handleClick}>
                  <li className={pagename == "/dashboard" ? "active" : ''}><Link to='/dashboard'>District at a
                    Glance</Link></li>
                  <li className={pagename == "/StrengthWeakness" ? "active" : ''}><Link to='/StrengthWeakness'>Strength
                    & Weakness</Link></li>
                  <li className={pagename == "/StudentPerformance" ? "active" : ''}><Link to='/StudentPerformance'>Student
                    Performance & Progress</Link></li>
                  {/* <li><Link to='/collegecareerreadiness'>College and Career Readiness</Link></li> */}
                  {/* <li><Link to='/humanResources'>Human Resources</Link></li> */}
                  <li className={pagename == "/enrollment" ? "active" : ''}><Link to='/enrollment'>Enrollment</Link>
                  </li>
                  {/* <li><Link to='/districtgoals'>District Goal & Strategy</Link></li> */}
                  <li className={pagename == "/studentbehavior" ? "active" : ''}><Link to='/studentbehavior'>Student
                    Behavior</Link></li>
                  <li className={pagename == "/schoolperformance" ? "active" : ''}><Link to='/schoolperformance'>School
                    Performance</Link></li>
                  <li className={pagename == "/statebenchmarking" ? "active" : ''}><Link to='/statebenchmarking'>State
                    Benchmarking</Link></li>
                  <li className={pagename == "/financedashboard" ? "active" : ''}><Link to='/financedashboard'>Financial
                    Dashboard</Link></li>
            ...
            <TabPanel header="" headerTemplate={SettingHeaderTemplate} headerClassName="flex align-items-center">
              <div className="bg-[#0d234c] p-5 h-[120px]">
                <div className="text-[18px] text-[#fff] mb-3" style={{cursor: "pointer"}}
                     onClick={hangleLogOut}>LogOut
                </div>
